...
#   legcord             -> the real Electron binary (no wrapper, no injected
#                          Chromium flags). Legcord already has its own
#                          internal performance presets in app.asar
#                          (balanced/memory/battery/voip/latency) and applies
#                          them based on the user's selection in Settings →
#                          Performance Mode. Injecting flags via wrapper was
#                          fighting those presets and broke the rendering.
#
# To tweak performance, use the in-app setting, NOT the wrapper.
# =============================================================================
...
# icon search in STEP 6 finds them at /usr/share/icons/hicolor/...
# ---------------------------------------------------------------------------
echo ""
...
# ---------------------------------------------------------------------------
# STEP 5: (REMOVED) — no wrapper, no injected Chromium flags.
# Legcord already has its own internal performance presets in app.asar and
# applies them based on the user's Settings → Performance Mode selection.
# Injecting external flags via wrapper was fighting those presets and broke
# the rendering. The binary ./AppDir/bin/legcord is left untouched as the
# real Electron binary, exactly like the official .deb does.
# ---------------------------------------------------------------------------
